...
# this method gets the string.  it finds all the minuses that are duplicated
# in a row and cut
    ...
    while index != -1:
        ...
        if len(s2) % 2 == 0:
            if start == 0 or finish == len(eq) - 1:
                ...
                eq = s1 + s3
            elif (not finder.is_digit(eq[start - 1])) \
                    and (not (eq[start - 1] == ')')) \
    ...
    for i in range(len(lst)):
        if finder.is_number(lst[i]) or finder.is_expression(lst[i]):
            if i == 0 or i == len(lst) - 1:
                pass
            elif finder.is_number(lst[i + 1]) \
                    or finder.is_expression(lst[i + 1]):
...
def operate(lst, i):
    ...
    if finder.is_middle_operator(lst[i]):
        res = operate_middle(lst, i)
    if finder.is_right_operator(lst[i]):
        res = operate_right(lst, i)
    if finder.is_left_operator(lst[i]):
        res = operate_left(lst, i)
...
def operate_middle(lst, i):
    if not finder.is_operator(lst[i]):
        raise Exception("not an operator")
    if i == len(lst) - 1:
        raise Exception("the operator " + str(lst[i]) +
    ...
    if finder.is_number(str(lst[i + 1])) \
            and finder.is_number(str(lst[i - 1])):

        return get_result(float(lst[i - 1]),
                          float(lst[i + 1]),
                          lst[i])

    elif finder.is_number(str(lst[i - 1])) \
            and finder.is_minus(str(lst[i + 1])):

        return get_result(float(lst[i - 1]),
                          float(operate_middle(lst, i + 1)),
                          lst[i])
...
def operate_left(lst, i):
    ...
    elif finder.is_minus(lst[i + 1]):
        return get_result(operate_middle(lst, i + 1), None, lst[i])
